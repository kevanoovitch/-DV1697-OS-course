...
bool Chopstick(int seat, int id)
{
    ...
    while (true)
    {
        ...
        if (seat % 2 != 0) // if odd
        {
            ...
            printf("Thread#%u Waiting and ready to grab RIGHT, professor at seat %u\n", id, seat);
            ...
            {
                // found right chopstick
                //  wait and Try left return false if fail

                printf("Thread#%u got RIGHT chopstick starting wait, professor at seat %u\n", id, seat);
                waitRandom(2); // wait 2-8s

                if (pthread_mutex_lock(&chopssticks[left]) != 0)
                {
                    printf("Thread#%u Didn't get LEFT chopstick, professor at seat %u\n", id, seat);
                    // failed to grab left
                    pthread_mutex_unlock(&chopssticks[right]); // release left
                    return false;
                }
