...
static void *init_matrix(void *params)
{
    ...
    // usleep(25); //Helps but is bad for execution time and doesn't solve issues in bigger sizes
    matmul_par((void *)args);
}
...
static void *matmul_par(void *params)
{
    ...
    Input: params (contains the row index to compute)

    Extract row index from params - done
    FOR each column j in matrix C DO
    Set C[row][j] = 0
    FOR each element k in the row of A and column of B DO
